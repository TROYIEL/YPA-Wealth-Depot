...
export default async function Portal({ searchParams }: { searchParams: Promise<{ request?: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl px-5 pb-20 pt-8 text-slate-900 sm:px-8">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-7">
      ...
      <div className="flex items-center gap-4"><span className="text-sm text-slate-600">{user.email}</span><form action="/api/portal/signout" method="post"><button className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold hover:bg-white">Sign out</button></form></div>
    </header>
    <div className="mt-10 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-widest text-blue-700">Member portal</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Welcome, {user.name || "member"}</h1><p className="mt-2 text-slate-600">Your YPA account at a glance</p></div><span className="rounded-full bg-blue-50 px-4 py-2 text-sm font-medium text-blue-800">{date(new Date().toISOString())}</span></div>
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-[1.6fr_1fr]">
      ...
      <div className="space-y-6">
        ...
        <section className={card}><h2 className="text-xl font-bold">My requests</h2><div className="mt-4 space-y-3">{requests.length ? [...requests].sort((a,b) => String(b.$createdAt).localeCompare(String(a.$createdAt))).slice(0,10).map(r => <div key={r.$id} className="flex justify-between gap-3 rounded-xl bg-slate-50 p-3 text-sm"><div><strong>{money(r.amount)}</strong><p className="text-slate-500">{date(r.$createdAt)}</p></div><span className="capitalize text-blue-800">{String(r.status)}</span></div>) : <p className="text-sm text-slate-500">No requests yet.</p>}</div></section>
